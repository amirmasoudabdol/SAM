...
public:
  ...
  Writer(const filesystem::path &path, const string &prefix, const filesystem::path filename);
  
  Writer(const filesystem::path &filename, const std::vector<std::string> colnames);
...
public:
  ...
  Reader(const filesystem::path &filename);

  void filename(const filesystem::path &name) {
    file_name_ = name;
