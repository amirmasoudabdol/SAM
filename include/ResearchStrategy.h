...
public:
  ...
  /// selectionBetweenSubmission, it's weird and I cannot just call it in any
  /// other methods because then it's hidden
  ///
 ...
 virtual std::optional<SubmissionPool> selectOutcomeFromPool(
  ...
  ///
  /// @param experiment a reference to the experiment
  /// @param pchain a policy chain, usually stored in `stashing_policy` in the
  /// config file
